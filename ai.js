...
async function tryNvidia(message, systemInstruction) {
  ...
  for (const m of [model, fallbackModel]) {
    try {
      ...
      console.warn(`[Pathey NVIDIA] (${m}) Failed:`, err.message);
    }
  }
  return null;
}

async function tryVercel(message, systemInstruction) {
  ...
  for (const ep of vercelEndpoints) {
    try {
      ...
      console.warn('[Pathey Vercel] Failed:', err.message);
    }
  }
  return null;
}
...
async function getAIResponse(message, memoryContext, preferredModel, history = []) {
  ...
  if (preferredModel === 'vercel') {
    const vReply = await tryVercel(message, systemInstruction);
  ...
  if (vercelReply) {
    conversationHistory.push({ role: 'user', content: message });
    conversationHistory.push({ role: 'assistant', content: vercelReply });
    while (conversationHistory.length > MAX_HISTORY * 2) conversationHistory.splice(0, 2);
    return vercelReply;
  }
